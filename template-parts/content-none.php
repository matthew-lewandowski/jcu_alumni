...
?>
    <header class="page-header">
        <h1 class="page-title">
            <?php
            if ( is_404() ) { esc_html_e( 'Page not available', 'jcu_alumni' );
            } else if ( is_search() ) {
                /* translators: %s = search query */
                printf( esc_html__( 'Nothing found for &ldquo;%s&rdquo;', 'jcu_alumni'), get_search_query() );
            } else {
                esc_html_e( 'Nothing Found', 'jcu_alumni' );
            }
            ?>
        </h1>
    </header><!-- .page-header -->
